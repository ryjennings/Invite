...
+ (void)insetGroupedTableView:(UITableView *)tableView cell:(UITableViewCell *)cell indexPath:(NSIndexPath *)indexPath
{
    if ([cell respondsToSelector:@selector(tintColor)]) {
        CGFloat cornerRadius = kGroupCornerRadius;
        cell.backgroundColor = UIColor.clearColor;
        CAShapeLayer *layer = [[CAShapeLayer alloc] init];
        CGMutablePathRef pathRef = CGPathCreateMutable();
        CGRect bounds = CGRectInset(cell.bounds, kGroupMargin, 0);
        BOOL addLine = NO;
        ...
        if (addLine == YES) {
            ...
            lineLayer.frame = CGRectMake(CGRectGetMinX(bounds)+kGroupMargin, bounds.size.height-lineHeight, bounds.size.width-kGroupMargin, lineHeight);
